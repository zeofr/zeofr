...
# GitHub fine-grained PAT — set as repository secret ACCESS_TOKEN
# Required scopes: read:Followers, read:Starring, read:Watching
# Repository: read:Commit statuses, read:Contents, read:Metadata
HEADERS = {'authorization': 'token ' + os.environ['ACCESS_TOKEN']}
USER_NAME = os.environ['USER_NAME']  # 'zeofr'
QUERY_COUNT = {
    'user_getter': 0,
    'follower_getter': 0,
    'graph_repos_stars': 0,
...
BIRTHDAY = datetime.datetime(2004, 8, 19)


def daily_readme(birthday):
    """
    Returns the time since birthday as a human-readable string.
    e.g. '20 years, 10 months, 18 days'
    """
...
def force_close_file(data, cache_comment):
    ...
    with open(filename, 'w') as f:
        f.writelines(cache_comment)
        f.writelines(data)
    print('Error: partial data saved to', filename)
...
def svg_overwrite(filename, age_data, acc_date, commit_data, star_data,
                  repo_data, contrib_data, follower_data, loc_data):
    tree = etree.parse(filename)
    root = tree.getroot()
    # No dot justification needed for Nothing UI — just plain text replacement
    find_and_replace(root, 'age_data',      str(age_data))
    find_and_replace(root, 'acc_date',      str(acc_date))
...
if __name__ == '__main__':
    ...
    # Format account creation date as e.g. "NOV 2024"
    acc_date_fmt = datetime.datetime.strptime(acc_date, '%Y-%m-%dT%H:%M:%SZ').strftime('%b %Y').upper()

    svg_overwrite('dark_mode.svg',  age_data, acc_date_fmt, commit_data, star_data,
                  repo_data, contrib_data, follower_data, total_loc[:-1])
    svg_overwrite('light_mode.svg', age_data, acc_date_fmt, commit_data, star_data,
                  repo_data, contrib_data, follower_data, total_loc[:-1])
